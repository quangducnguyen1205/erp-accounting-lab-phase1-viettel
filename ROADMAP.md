...
| **Tổng task** | 79 |
| **Đã hoàn thành** | 47 / 79 |
| **Focus hiện tại** | Flyway rollback/failure handling |
| **Milestone tiếp theo** | #7 - Flyway rollback/failure handling |
...
| 7 | 19/05 | Flyway rollback/failure handling | Kế tiếp |
...
### Task tiếp theo: Flyway rollback/failure handling

1. Mở/tạo `docs/03-backend-database-mo-rong/flyway-rollback-failure-handling.md`.
2. Đọc nguồn chuẩn Flyway về versioned migration, schema history, validate, repair, failure và undo migration.
3. Chỉ tạo mini-lab nếu an toàn trên local DB; không phá schema demo chính nếu chưa có backup/reset path rõ.
4. Không chuyển sang ACID/Keycloak trước khi chốt summary Flyway failure/rollback.
