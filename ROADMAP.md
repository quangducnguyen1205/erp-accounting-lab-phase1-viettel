...
Mở trước: `lab-code/tenant-demo/pom.xml`.

Task tiếp theo:

1. Kiểm tra skeleton trong `lab-code/tenant-demo/`: `pom.xml`, `application.yml`, `TenantDemoApplication.java`, thư mục `db/migration/`.
2. Tự hoàn thiện bootstrap tối thiểu cho Spring Boot: Web, JPA, PostgreSQL, Flyway và test dependencies.
3. Tạo/hoàn thiện Flyway baseline `V1`, `V2`, `V3` dựa trên SQL playground, không để Hibernate tự tạo schema.
4. Verify bằng: `cd lab-code && make db-up && make sql-reset && make app-run`.
5. Done khi app khởi động được, Flyway migrate schema thành công, và log không báo lỗi kết nối database.
