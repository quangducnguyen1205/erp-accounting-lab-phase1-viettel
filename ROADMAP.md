...
| **Tổng task** | 79 |
| **Đã hoàn thành** | 43 / 79 |
| **Focus hiện tại** | PostgreSQL index query-pattern mini-lab |
| **Milestone tiếp theo** | #6 - PostgreSQL index query-pattern mini-lab |
...
| 6 | 17/05 | PostgreSQL index query-pattern mini-lab | Kế tiếp |
...
### Task tiếp theo: PostgreSQL index query-pattern mini-lab

1. Mở `docs/03-backend-database-mo-rong/index-query-patterns-postgresql.md`.
2. Mở `lab-code/sql-playground/07-index-query-patterns.sql`.
3. Tự điền từng TODO: tạo bảng tạm, sinh dữ liệu, tạo/drop index, chạy `EXPLAIN`.
4. Chỉ tập trung prefix `LIKE`, contains `LIKE`, expression index, composite index và tenant-aware query.
5. Không chuyển sang Flyway/ACID/Keycloak trước khi tự chạy xong lab 07 hoặc ghi rõ blocker.
