...
| **Tổng task** | 48 |
| **Đã hoàn thành** | 24 / 48 |
| **Focus hiện tại** | Tổng kết API tenant-aware + test chống data leakage |
| **Milestone tiếp theo** | #4 — Tenant-aware API Demo |
...
- `lab-code/tenant-demo/` đã khởi động được với Spring Boot, Flyway baseline, TenantFilter, entity/repository và API `master_data` tenant-aware cơ bản. Test backend chống data leakage chưa làm.
...
| Tenant-aware repository/service/controller | Core / must practice | MasterData API + curl proof | #4 | API + curl proof + Agent review xong |
| Basic verification / curl / tests | Core / must practice | curl commands + `make app-test` | #4, #5, #7 | Curl API xong, test chưa |
...
- [x] `[LÝ THUYẾT]` Đọc `docs/03-backend-database-mo-rong/migration-lock-rollback.md` — output: 5-7 bullet về safe vs risky migration trong `docs/99-tong-ket/nhung-gi-da-nam-duoc.md`; không overdo thành guide production
- [x] `[THỰC HÀNH]` Tự tạo khi bắt đầu task: `lab-code/sql-playground/06-migration-lock-observation.sql` — thử vài `ALTER TABLE` nhỏ trên `master_data`; verify baseline bằng `cd lab-code && make db-up && make sql-reset && make sql-all`
- [x] `[BÁO CÁO]` Ghi summary ngắn cho Milestone #2: lock là gì, vì sao migration ảnh hưởng nhiều tenant, rollback cần chuẩn bị gì
...
- [x] `[MILESTONE]` Chốt Milestone #2 — PostgreSQL safety summary + output thực hành nhỏ
...
- [x] `[THỰC HÀNH]` Tự code `MasterDataService.java` và `MasterDataController.java` — endpoint đọc danh sách và tìm theo `code` trong tenant hiện tại
- [x] `[THỰC HÀNH]` Verify bằng curl: `X-Tenant-Id: 1` chỉ thấy data tenant 1; `X-Tenant-Id: 2` chỉ thấy data tenant 2; ghi command/output ngắn
- [x] `[REVIEW]` Sau khi tự chạy được, nhờ Agent review focused: có query nào quên `tenantId` không, service/controller có nhận tenant sai nguồn không
...
Mở trước: `docs/99-tong-ket/nhung-gi-da-nam-duoc.md`.

Task tiếp theo:

1. Ghi 3 rule backend tenant-aware: tenant lấy từ trusted context, repository query luôn scoped theo `tenantId`, không tin `tenant_id` từ request body.
2. Ghi curl pattern ngắn cho Milestone #4: tenant 1 chỉ thấy data tenant 1, tenant 2 chỉ thấy data tenant 2, cross-tenant id trả `404`.
3. Sau đó tự code `DataLeakageTest.java` để biến curl proof thành test tự động.
4. Verify bằng: `cd lab-code && make app-test`.
5. Done khi summary ngắn đã có, test pass hoặc có blocker cụ thể và không mở thêm feature mới.
