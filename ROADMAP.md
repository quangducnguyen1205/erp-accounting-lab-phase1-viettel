...
- Redis/MinIO/Elasticsearch/Kafka/Observability tiếp tục học theo thứ tự sau Keycloak, mỗi công nghệ có concept + code guide + mini-lab nhỏ.
...
- React UI: optional, chỉ làm nếu cần demo trực quan hơn backend script.
...
- React UI để optional/later; nếu cần demo, backend script hiện đã đủ làm fallback.
...
| Keycloak/OAuth2/OIDC | Mini-lab | `docs/05-security/keycloak-oidc-mental-model.md`, `docs/05-security/keycloak-oauth2-oidc-awareness.md`, `docs/05-security/keycloak-admin-console-guide.md` | `lab-code/keycloak-lab/`, `APP_AUTH_MODE=keycloak` | Lấy token Keycloak, gọi API tenant-aware, verify issuer/JWKS/claims | #9 | Đã verify mini-lab |
...
| Debezium CDC + Kafka | Awareness | `docs/07-architecture/debezium-cdc.md` | Không chạy CDC | CDC role summary | #14/#17 | Later awareness |
...
| Observability: Prometheus/Grafana/Loki | Important mini-lab/note | `docs/07-architecture/observability-prometheus-grafana-loki.md` | Actuator/log/metric pattern, optional stack | Explain metric/log/tracing role | #15 | Planned |
...
| DDD/domain boundaries | Later awareness | `docs/08-design/ddd-awareness.md` | Không refactor code theo DDD ở Phase 1 | Post-demo design note | #17 | Later |
...
- [ ] `[MILESTONE]` Chốt Milestone #12 - MinIO mini-lab có upload/download evidence và giới hạn production.

### Sprint 13 - 25/05: Redis/cache mini-lab
...
- [ ] `[MILESTONE]` Chốt Milestone #13 - Redis mini-lab có cache key pattern và summary.

### Sprint 14 - 26/05: Kafka/async messaging mini-lab

Mục tiêu: hiểu async/event-driven communication ở mức nhỏ, không chạy full event platform nếu chưa cần.

- [ ] `[LÝ THUYẾT]` Tạo `docs/07-architecture/kafka-async-messaging.md` và `docs/07-architecture/kafka-code-guide-spring-boot.md` - producer, consumer, topic, event contract, retry/idempotency awareness.
- [ ] `[SKELETON]` Nếu máy chịu được Docker: chuẩn bị `lab-code/kafka-lab/` và package `com.viettel.demo.messaging`; nếu quá nặng, tạo code skeleton + sequence diagram awareness.
...
- [ ] `[MILESTONE]` Chốt Milestone #14 - Kafka/async có flow, caveat và quyết định rõ chạy thật hay awareness.

### Sprint 15 - 27/05: Observability/logging/metrics mini-lab

Mục tiêu: biết app production cần log/metric/health thế nào, nhưng không dựng full Grafana stack nếu không cần.

- [ ] `[LÝ THUYẾT]` Tạo `docs/07-architecture/observability-prometheus-grafana-loki.md` và code guide ngắn: logs, metrics, traces, health, correlation id.
...
- [ ] `[MILESTONE]` Chốt Milestone #15 - observability summary đủ giải thích trong target architecture.

### Sprint 16 - 28/05: API Gateway/service discovery awareness + React optional decision
...
- [ ] `[MILESTONE]` Chốt Milestone #16 - architecture awareness đủ trả lời mentor, không cần chạy full gateway.

### Sprint 17 - 29/05 đến 01/06: DDD awareness + final reflection
...
| 15 | 27/05 | Observability/logging/metrics mini-lab | Planned |
| 16 | 28/05 | API Gateway/service discovery awareness + React decision | Planned |
| 17 | 01/06 | DDD awareness + final reflection + demo dry-run | Planned |
...
| Observability mini-lab | `docs/07-architecture/observability-prometheus-grafana-loki.md` - tạo khi tới Sprint 15 |
...
- Kafka/Debezium/Observability stack đầy đủ: chỉ làm khi tới đúng sprint và giữ scope nhỏ.
