...
| **Tổng task** | 81 |
| **Đã hoàn thành** | 66 / 81 |
| **Focus hiện tại** | Sprint 11 - Elasticsearch/search mini-lab sau PostgreSQL query-pattern |
| **Milestone tiếp theo** | #11 - architecture coverage + search mini-lab |
| **Demo cuối Phase 1** | Spring Boot + PostgreSQL/Flyway + tenant-aware API + JWT tạm + Keycloak mini-lab đã verify |
...
| Elasticsearch / Elastic Stack | Mini-lab khi có search need | `docs/07-architecture/elasticsearch-search-service.md` | `lab-code/elasticsearch-lab/` + search skeleton | Search behavior summary | #11 | Đang bắt đầu |
...
- [x] `[SKELETON]` Nhờ Codex scaffold `lab-code/tenant-ui/` với TODO comments nếu chọn UI; nếu không, tạo demo script backend chi tiết.
...
- [x] `[LÝ THUYẾT]` Tạo `docs/07-architecture/target-architecture-adoption-map.md` - map React, gateway, Keycloak, backend services, PostgreSQL, Redis, Kafka, MinIO, Elastic, observability, LLM, external systems vào vai trò tổng thể.
- [ ] `[LÝ THUYẾT]` Tạo/read `docs/07-architecture/elasticsearch-search-service.md` và `docs/07-architecture/elasticsearch-mini-lab-plan.md` - vì sao search engine xuất hiện sau PostgreSQL `LIKE`/index query pattern.
- [ ] `[SKELETON]` Chuẩn bị skeleton Elasticsearch/search cho `master_data`: docker compose local, config disabled mặc định, TODO code package, HTTP Client; không làm Redis/MinIO/Kafka cùng lúc.
- [ ] `[THỰC HÀNH]` Tự code mini-lab: index `master_data`, search keyword tenant 1/tenant 2, verify không leak cross-tenant, ghi lại eventual consistency caveat.
...
| 9 | 24/05 | Keycloak/OIDC mini-lab hoặc awareness evidence | Đã đóng |
| 10 | 26/05 | React UI nhỏ hoặc backend demo script chắc chắn | Đang làm |
| 11 | 29/05 | Target architecture adoption map | Đang làm |
...
| Elasticsearch/search mini-lab | `docs/07-architecture/elasticsearch-search-service.md`, `docs/07-architecture/elasticsearch-mini-lab-plan.md`, `lab-code/elasticsearch-lab/`, `lab-code/tenant-demo/src/main/java/com/viettel/demo/search/` |
...
### Task tiếp theo: chọn đường demo cho Milestone #10

1. Đọc lại `docs/99-tong-ket/nhung-gi-da-nam-duoc.md` phần Milestone #9 để nắm flow đã verify.
2. Ưu tiên backend demo script trước vì Keycloak tenant flow đã verify và ít mở thêm scope hơn React.
3. Mở `presentation-notes/demo-script-keycloak-tenant-flow.md` và dry-run từng bước: PostgreSQL, Keycloak, `APP_AUTH_MODE=keycloak`, token tenant 1/2, API verify, cross-tenant id, `make app-test`.
4. Nếu dry-run ổn và còn thời gian: tạo/read `docs/06-frontend/react-tenant-demo-ui.md`, rồi mới quyết định scaffold `lab-code/tenant-ui/`.
5. Done khi có một đường demo mentor-facing chạy lại được, không cần full frontend production.
