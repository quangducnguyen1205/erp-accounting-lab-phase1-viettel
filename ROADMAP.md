...
| **Tổng task** | 80 |
| **Đã hoàn thành** | 64 / 80 |
| **Focus hiện tại** | Sprint 10 - React UI nhỏ hoặc backend demo script chắc chắn |
...
| 10 | 26/05 | React UI nhỏ hoặc backend demo script chắc chắn | Kế tiếp |
...
2. Quyết định Milestone #10 đi theo UI React nhỏ hay backend demo script chắc chắn.
3. Nếu chọn UI: tạo/read `docs/06-frontend/react-tenant-demo-ui.md`, rồi scaffold `lab-code/tenant-ui/` với TODO comments.
4. Nếu chưa kịp UI: tạo demo script backend gồm `make db-up`, Keycloak token flow, `APP_AUTH_MODE=keycloak make app-run`, HTTP/curl verify tenant 1/2, missing/invalid token, cross-tenant id.
