...
| 8 | 21/05 | ACID/isolation levels observation | Kế tiếp |
...
### Task tiếp theo: ACID và isolation levels

1. Tạo/read `docs/03-backend-database-mo-rong/acid-isolation-levels-postgresql.md`.
2. Tập trung ACID, isolation levels, dirty read, non-repeatable read, phantom read và PostgreSQL default isolation.
3. Chuẩn bị skeleton `lab-code/sql-playground/09-acid-isolation-observation.sql` nếu cần quan sát hai session.
4. Chưa chuyển sang Keycloak/React trước khi có summary ACID/isolation ngắn.
