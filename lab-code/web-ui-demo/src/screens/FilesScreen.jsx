...
export function FilesScreen({
  ...
  const columns = useMemo(() => [
    { key: 'originalFilename', label: 'Tên tệp tin' },
    { key: 'contentType', label: 'Loại' },
    { key: 'sizeBytes', label: 'Dung lượng', render: (row) => formatSize(row.sizeBytes) },
    { key: 'createdAt', label: 'Tải lên lúc', render: (row) => formatDateTime(row.createdAt, 'Vừa tải lên') },
    {
      key: 'actions',
      label: 'Thao tác',
      render: (row) => (
        <div className="row-actions">
          <button type="button" className="button-secondary button-sm" onClick={() => onDownload(row)} disabled={disabled || loading}>
            Tải xuống
          </button>
          <button type="button" className="button-danger button-sm" onClick={() => onDelete(row)} disabled={disabled || loading}>
            Xóa
          </button>
        </div>
      )
  ...
  async function submitUpload(event) {
    ...
    setUploadHint('');
    if (!selectedFile) {
      setUploadHint('Hãy chọn tệp tin trước khi tải lên.');
  ...
  return (
    <div className="screen-grid">
      <section className="screen-heading">
        <div className="screen-heading-row">
          ...
          <div className="screen-heading-actions">
            <button type="button" onClick={() => setUploadOpen(true)} disabled={disabled || loading || isViewer}>
              Tải tài liệu lên
            </button>
          </div>
        </div>
      ...
      <section className="panel panel-span-3 files-list-panel">
      ...
      <Drawer open={uploadOpen} onClose={() => setUploadOpen(false)} title="Tải tài liệu lên">
        {isViewer && (
          <Alert tone="warning" title="Vai trò VIEWER">
            Viewer có thể xem và tải tệp tin xuống, nhưng không được tải lên hoặc xóa.
          </Alert>
        )}
        <form className="drawer-form" onSubmit={submitUpload}>
          ...
          <div className="drawer-actions">
            <button type="button" className="button-secondary" onClick={() => setUploadOpen(false)}>Hủy</button>
