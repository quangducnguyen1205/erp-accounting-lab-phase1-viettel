...
> Cập nhật sau JWT tạm: note này ban đầu dùng `X-Tenant-Id` để học filter/header flow. `DataLeakageTest.java` hiện đã chuyển sang `Authorization: Bearer <token>` và `JwtTenantContextFilter`. Ý nghĩa test không đổi: request vẫn phải đi qua context layer trước khi tới service/repository, chỉ khác nguồn tenant hiện tại là JWT claim đã validate.
