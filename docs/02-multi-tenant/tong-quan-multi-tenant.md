...
```mermaid
graph TB
    PLATFORM["Nền tảng ERP/Kế toán SaaS"]
    PLATFORM --> TA["Tenant A<br/>Công ty A"]
    PLATFORM --> TB["Tenant B<br/>Công ty B"]
    PLATFORM --> TC["Tenant C<br/>Công ty C"]

    TA --> TA_D["users / data / config"]
    TB --> TB_D["users / data / config"]
    TC --> TC_D["users / data / config"]

    style PLATFORM fill:#1565c0,color:white
...
sequenceDiagram
    ...
    Auth->>Ctx: Extract tenant_id từ token
    Ctx->>Svc: Forward (tenant_id = 42)
    Svc->>Cache: GET tenant:42:categories
    alt Cache hit
        Cache-->>Svc: Trả data
    else Cache miss
        Svc->>DB: SELECT ... WHERE tenant_id = 42
        DB-->>Svc: Result
