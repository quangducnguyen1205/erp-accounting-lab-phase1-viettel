...
Điểm dễ nhầm: log không phải endpoint để mình gọi như `/actuator/metrics`. App chủ động emit log ra console/stdout/file hoặc hệ thống thu log bên ngoài. Trong local lab hiện tại, mình đọc log trực tiếp ở terminal đang chạy app.
