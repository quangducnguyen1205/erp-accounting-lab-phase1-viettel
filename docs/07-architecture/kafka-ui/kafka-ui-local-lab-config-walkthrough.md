...
Vì vậy target `make kafka-ui-up` chạy `kafka-up` trước để broker và network tồn tại.
...
```bash
make kafka-ui-up       # start Kafka broker nếu cần, rồi start Kafka UI
make kafka-ui-status   # xem container Kafka UI
make kafka-ui-logs     # xem log Kafka UI
make kafka-ui-down     # dừng Kafka UI, không dừng Kafka broker
...
- Chưa chạy `make kafka-up` nên external network `viettel-kafka-net` chưa tồn tại.
