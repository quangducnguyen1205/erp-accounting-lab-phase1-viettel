...
1. Final React Web demo polish sau khi backend boundaries ổn.
2. Optional: audit-events screen để xem kết quả cross-service Kafka từ UI.

Loki/Grafana log aggregation, Kafka UI, Kong Gateway và `audit-log-service` đã có local lab. Cross-service Kafka flow đã verify, nên bước hợp lý tiếp theo là polish UI vừa đủ để demo service split.
