...
### Loki/log aggregation local lab

Trạng thái: đã có local Docker lab ở `lab-code/loki-lab/` dùng Loki + Grafana + Grafana Alloy.

Đã nắm được:

- app/container ghi log ra stdout;
- Alloy đọc Docker container logs qua Docker socket;
- Alloy forward logs sang Loki;
- Grafana Explore query Loki datasource;
- label phù hợp là `service`, `container`, `compose_project`, `environment`;
