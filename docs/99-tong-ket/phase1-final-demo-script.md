...
| Search API through Kong | `http://localhost:18000/api/search/master-data?keyword=...` |
...
3. Đợi vài giây để Kafka event được `search-service` consume và index.
4. Trong panel `Tìm kiếm nâng cao`, tìm theo code hoặc tên vừa tạo.
5. Kỳ vọng kết quả chỉ chứa record của tenant hiện tại.
6. Login `tenant2-user/password` và tìm cùng keyword: tenant 2 không thấy record tenant 1.
7. Mở Loki query:

```logql
{service="search-service"}
{service="search-service"} |= "SEARCH-DEMO"
```

Điểm cần nói: PostgreSQL vẫn là source of truth; Elasticsearch là projection eventually consistent. UI gọi Kong tới `search-service`, không gọi Elasticsearch trực tiếp.
...
| Elasticsearch search | `GET /api/search/master-data?keyword=...` | UI qua Kong | Có | `search-service/http/search-api.http` | Ready nếu `search-service` + Elasticsearch chạy |
...
Elasticsearch/search hiện là `search-service` event-driven projection. File upload/download và search đều có UI mỏng vì đây là business feature tự nhiên của `Master Data Portal`.
...
| search-service | Consume Kafka event, update Elasticsearch projection, expose tenant-aware search API. |
