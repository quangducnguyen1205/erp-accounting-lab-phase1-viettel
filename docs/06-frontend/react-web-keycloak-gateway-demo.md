...
- CSS đơn giản.
- Multi-screen SPA bằng state nội bộ: Welcome, Dashboard, Master Data, Audit Events, Observability.
...
9. Audit Events: bấm `Load audit events` để gọi `GET /api/audit-events` qua Kong tới `audit-log-service`.
10. Observability: dùng links/LogQL recipes để mở Grafana Loki/Kafka UI.
