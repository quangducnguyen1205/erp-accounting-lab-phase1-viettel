...
- CSS đơn giản.
- Multi-screen SPA bằng state nội bộ. Product direction mới là `Master Data Portal`: Welcome, Dashboard, Master Data, Files/Tệp tin, Activity Log, Account.
...
10. Activity Log: bấm `Load activity` để gọi `GET /api/audit-events` qua Kong tới `audit-log-service`.
11. Demo docs: dùng URL/LogQL trong final demo script để mở Grafana Loki/Kafka UI khi cần giải thích backend flow.
