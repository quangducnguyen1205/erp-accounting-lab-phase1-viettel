...
Các ghi chú chi tiết được duy trì trong các file markdown của repository. Báo cáo này là bản tóm tắt phục vụ báo cáo tiến độ; repository đóng vai trò tài liệu đính kèm và nguồn sự thật cho các phần kiến thức đã được tổng hợp.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Cache key phải có tenant prefix, ví dụ \texttt{tenant:42:categories}.
    \item Platform admin và tenant admin là hai phạm vi quyền khác nhau, không nên trộn lẫn.
\end{itemize}

\section*{5. Bài học backend/database}

Em đã bắt đầu đi sâu vào các chủ đề database liên quan trực tiếp đến multi-tenant:

\begin{itemize}[leftmargin=*]
    \item PostgreSQL phù hợp với hệ thống kế toán vì hỗ trợ ACID transaction, constraint, schema và query planner tốt.
    \item Trong shared table, composite index thường nên bắt đầu bằng \texttt{tenant\_id}, ví dụ \texttt{(tenant\_id, status)} hoặc \texttt{(tenant\_id, created\_at)}.
    \item Query thiếu tenant filter vừa nguy hiểm về bảo mật, vừa dễ gây full table scan.
    \item Noisy neighbor có thể xảy ra ở CPU, disk I/O, shared buffers, connection pool và VACUUM.
    \item Migration cần chú ý lock. Một số lệnh \texttt{ALTER TABLE} có thể ảnh hưởng tất cả tenant nếu bảng dùng chung.
    \item Rollback database khó hơn rollback code, nên migration cần chia nhỏ và tương thích ngược.
\end{itemize}

\section*{6. Giới hạn hiện tại và hướng học tiếp}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Hiểu sâu hơn về PostgreSQL locking và long-running transactions.
    \item Thực hành migration an toàn bằng Flyway hoặc Liquibase.
...
Phase 1 đã giúp em hình thành nền tảng rõ hơn về SaaS, multi-tenant và các vấn đề backend/database đi kèm. Em đã hiểu rằng thiết kế SaaS không chỉ là đưa ứng dụng lên cloud, mà là một chuỗi quyết định liên quan đến cách ly dữ liệu, migration, deployment, vận hành và khả năng mở rộng.

Giai đoạn tiếp theo nên tập trung vào thực hành PostgreSQL và backend demo nhỏ, để chuyển kiến thức đã tổng hợp thành kỹ năng triển khai và kiểm chứng bằng code.
